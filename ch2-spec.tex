...
В спеке \code{s/coll-of} ключи \verb|:count|, \verb|:min-count| и
\verb|:max-count| проверяют число элементов в списке: точное, минимальное и
...
\verb|:min-count| единицу, мы отсечём пустоту:
...
(s/valid? ::ne-url-list ["http://test.com"])
;; true
  \end{clojure}
\end{english}

Как и в случае со строкой, префикс \code{ne-} означает <<не пустой>>, только на
этот раз он относится к списку.
