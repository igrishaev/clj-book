...
\coderef{2}. Поместим его в словарь с ключом \texttt{:this}. Затем~--- одна и
более любых команд, включая вложенные \texttt{if} \coderef{3}. Здесь мы просто
...
встретится тег \verb|else| с последующими командами \coderef{4}. Спека
завершается элементом \verb|end| \coderef{6}.
...
Чтобы наконец-то выполнить код, пойдём на несколько упрощений. Во-первых,
