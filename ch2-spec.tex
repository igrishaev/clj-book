...
В отч\"{е}те все, что нам нужно. Из \spverb|:val| возьмем проблемное
...
Вызво \spverb|get-message| с этой верн\"{е}т сообщение о пустой строке. Попробуем
...
документации (ее может и не быть). Далее~--- тело или список тел. Набросаем
черновик:
...
\spverb|:defn/body| так, чтобы она учитывала оба случая. Обозначим ее
...
ее поведение изменилось: кроме документации она выводит спеку, если ее
...
Предположим, конфигурация описана в EDN-файле. Поле \spverb|:db| задает
подключение к базе.
...
Это все еще машинный текст, который нельзя показывать пользователю. Вс\"{е} же
...
лежит объект \spverb|Spec|. Он оборачивает стандартную спеку и дополняет ее
...
генераторы, мульти-спеки и другие интересные вещи. Все это читатель найдет в
