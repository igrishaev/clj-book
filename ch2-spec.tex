...
найдётся условный \spverb|unsigned int|, который охватит именно этот
...
В ООП знают об этой проблеме и решают её классами \spverb|UnixPort| и
...
\spverb|new UnixPort(-42)| привело к ошибке компиляции. Найти её могут
...
Синоним \spverb|s| нужен, чтобы избежать конфликтов имён с
\spverb|clojure.core|. Модуль Spec несёт макросы \spverb|s/and|, \spverb|s/or| и
...
Вторым аргументом идёт предикат \spverb|string?|. Предикат это функция,
...
него можно сослаться: функция \spverb|s/get-spec| по~ключу спеки вернёт
...
стилю. Но ещё лучше комбинировать не предикаты, а спеки. Макрос \spverb|s/and|
...
оборвётся. Всё, что после \spverb|::ne-string| не сработает, и
исключения не будет.
...
\spverb|:req-un| и какие ещё параметры принимает \spverb|s/keys|.
...
\spverb|int?|, что приведёт к ошибке. Это тот случай, когда пустое значение
...
До сих пор мы проверяли данные с помощью \spverb|s/valid?|. Функция вернёт
...
нет смысла парсить число: подойдёт \spverb|case| или словарь.
...
выражение тела. В противном случае вернётся \spverb|::s/invalid|. Мы
...
\noindent
Переменная \spverb|->lower| это обёртка для приведения регистра:
...
он~бросит исключение. \spverb|With-conformer| перехватит его и вернёт
\spverb|::s/invalid|.
...
\noindent
Теперь \spverb|s/conform| вернёт не просто значение, а пару с тегом:
...
вернёт исходное значение. Выше мы получили словарь, где поле \spverb|:port| это
...
поэтому для предиката \spverb|unform| отбросит тег и вернёт значение. Тег
...
\spverb|conform| и \spverb|unform| всё пошло не так. \spverb|Conform| вывел тип,
...
вернёт то, что получили на этапе \spverb|conform|. Полный цикл
...
\item
  \spverb|s/explain-str| возвращает отчёт в виде строки;

\item
  \spverb|s/explain-data| возвращает словарь. Это самый полный отчёт об ошибке.
...
\spverb|not-empty|. Ошибка случилась на этапе \spverb|::string|, о чём и было
сказано.
...
меньше понятен \spverb|explain|. Когда в коллекции больше трёх уровней,
...
В отчёте всё, что нам нужно. Из \spverb|:val| возьмём проблемное
...
Вызов \spverb|get-message| с этой ошибкой вернёт сообщение о пустой
...
ничего не нашли, \spverb|::sample|. Назовём метод восходящим поиском.
...
описание книги>>. Аналогично ведёт себя \spverb|::movie|~--- если передать
...
\spverb|ne-string| вернёт ложь, \spverb|:book/description| станет последним
...
значение test>>. Сообщение легко получить функцией \spverb|format|. В нём
...
Приведём пример, когда \spverb|:in| заканчивается на число. Пусть задана
...
сокращённый отчёт \spverb|s/explain-data|:
...
Вопрос откуда читать локаль остается на ваше усмотрение. Можно хранить её в
...
выводим типы из строк. В случае ошибки получим отчёт \spverb|explain|. Для
...
других спек. На вход подают коллекцию, и \spverb|s/cat| накладывает её
...
Добавим её в итоговую \spverb|s/cat|, но укажем, что она встречается ни разу
...
\spverb|([title], (key=value)*)*|, где звёздочка означает сколько угодно раз, в
том числе ничего.
...
\spverb|doall| насильно перебирает её, чтобы считать все элементы. Без
...
Перейдём к недостающим элементам. Функция \spverb|clear-ini-lines| убирает
...
Разберём устно, как бы мы построили спеку для разбора \spverb|defn|. Это список,
...
Теперь обернём спеку \spverb|:defn/body| так, чтобы она учитывала оба
случая: одно тело и несколько. Обозначим её звёздочкой. Поскольку
\spverb|:defn/body| это \spverb|regex|-спека, сгруппируем её с помощью
\spverb|s/spec|.
...
используют функцию \spverb|compare|, которая вернёт -1, 0 и 1 для случаев
...
\spverb|s/explain-str|. В поле \spverb|data| отчёт из
...
Подключение к базе задают словарём с ключами \spverb|:host|, \spverb|:port|,
...
Предположим, конфигурация описана в EDN-файле. Поле \spverb|:db| задаёт
подключение к базе.
...
\spverb|s/or|, которые порождают развилки. Если передать её в
...
Clojure. \spverb|Expound| подойдёт для проверки конфигурации на старте
...
лежит объект \spverb|Spec|. Он оборачивает стандартную спеку и дополняет её
