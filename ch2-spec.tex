...
\tuple{номер, емейл, статус}. Все значения строки. Для каждого пользователя
требуется:
...
скобках и пар \tuple{поле = значение}. Пример условного \spverb|config.ini|:
...
\item
  разбить пары \tuple{поле = значение};
...
секцией понимают заголовок и произвольное число пар \tuple{поле = значение}.
Запишем ее в виде \spverb|s/cat|:
