...
Рассмотрим задачу, которую автор решал в работе над библиотекой
\footurl{Etaoin}{https://github.com/igrishaev/etaoin}[Etaoin]. Один из
ее модулей выполняет команды Selenium IDE. Это расширение для браузера и один из
продуктов одноименной компании. Расширение записывает действия в браузере в файл
JSON. Получается сценарий, который можно запустить вручную или с помощью
утилиты. Представим, что наша задача~--- написать интерпретатор этих сценариев.

Внутри JSON-файла нас ожидает поле \verb|commands| с массивом команд. Каждая
команда~--- словарь с ключами \verb|id|, \verb|command|, \verb|target|,
\verb|value| и другими. Наиболее важно для нас поле \verb|command|, в котором
находится имя команды: <<open>>, <<click>>, <<assert>> и другие. Приведем
отрывок из файла:
...
На первый взгляд всё просто: обойдем команды в цикле и для каждой вызовем
мультиметод \verb|do-cmd|. Мультиметод находит реализацию по полю
\verb|:command|. Останется наполнить его всеми командами, и задача готова.
...
Ситуация усложняется из-за так называемых
\footurl{flow-команд}{https://www.selenium.dev/selenium-ide/docs/en/introduction/control-flow}[Control\\*flow]:
условия и повтора. В отличии от обычных команд, они включают себя другие (дочерние)
команды. Их нельзя выполнить в лоб: при обработке \verb|if| мы сперва проверяем
условие, а потом переходим на нужную ветку. В случае с for или repeat внутренний
блок нужно выполнить определенное число раз.

\index{control flow}

Ниже показан минимальный пример с командой \texttt{if}. С точки зрения данных
эти команды — вектор словарей, но мы добавили отступы для читаемости. Чтобы код
не вышел громоздким, оставим с каждой команде минимальный набор полей.
...
Как в настоящем языке программирования, команды Selenium IDE могут быть
...
задачу досконально, но разберем ее основную часть~--- группировку команд с
учетом flow-операторов. И поможет нам в этом спека, а именно ее
regex-функциональность.

\index{парсинг}

Очевидно, набор команд с \texttt{if} или \texttt{for} нельзя выполнить линейным
обходом. Условные операторы наделяют код структурой, и мы должны отразить
это. Для этого приведем плоский список к более сложному виду. Это будет список
пар, где первый элемент~--- тег, а второй~--- команда или ее сгруппированный
вариант. Например, для обычного \texttt{print} получим пару:
...
Видно, что второй элемент включает в себя все команды до завершающего
end. Обработать такой узел гораздо легче, потому что он содержит все
необходимое: головной \texttt{if}, команды из положительной и негативной веток.

Начнем код с простой функции \texttt{parse-flow}, которая принимает вектор
...
Определим спеку \texttt{::flow}. Это один и более элементов, каждый из которых
либо \texttt{if}, либо обычная команда (для краткости пока что не будем
учитывать \texttt{for}). Спека не допустит пустого списка команд, потому что
...
сказали, что не оператор ветвления. Чтобы отличить обычные команды от служебных,
...
\noindent
и объявим спеку \texttt{::command}. Она проверяет, что поле \texttt{:command}
...
Введем вспомогательную функцию \texttt{tag=}. Она принимает тег и возвращает
функцию, которая принимает команду и проверяет, что у нее такой же тег. Она
пригодится, чтобы описывать предикаты без вспомогательных функций:
...
Самое интересное кроется в спеке \texttt{::flow-if}. Приведем ее описание:
...
более любых команд, включая вложенные \texttt{if}. В поле \texttt{:else} попадет
негативная ветка, однако лишь в том случае, если она нашлась (обертка
\verb|s/?|~--- ни разу или один раз). Ветка обнаружится, если встретился тег
else с последующими командами. Спека завершается элементом end.

Уже на этом этапе можно проверить парсинг. Подберем минимальный пример с
командой печати и условием:
...
\noindent
Видно, что структура кода сохраняется:
...
Еще более важно, что ошибки в структуре не пройдут валидацию спекой. Например,
...
ровно как и пустая ветка. Все варианты ниже вернут \texttt{::s/invalid}.
...
Исследуйте отчет \verb|s/explain| для этих случаев и подумайте над показом
ошибки.

Парсинг легко дополнить новым оператором ветвления. До сих пор мы игнорировали
тег for для цикла, и теперь добавим его в три шага.

Во-первых, убедимся, что он входит в множество \texttt{flow-tags}, чтобы for не
считался обычной командой.

Во-вторых, расширим главную спеку \texttt{::flow}. Теперь ее элемент либо
...
Пример с циклом, в теле которого условие:
...
По аналогии легко добавить операторы \texttt{repeat} и другие, которые
...
и более блоков case; внутри каждого блока~--- произвольные команды, которые
выполняются, если управление переходи на этот блок.
...
Мы выполнили большую работу~--- из плоского списка команд вывели структуру с
учетом вложенности. Интересно, для этого понадобился только один модуль из
...
\texttt{print}. Начнем с общей функции \texttt{run-data}:
...
Она принимает список команд и парсит их \texttt{parse-flow}, которую мы задали
выше. Если парсинг не удался, напечатаем ошибку. Если же все в порядке, запустим
выполнение с помощью пока что неизвестной функции \texttt{run-flow}. Напомним,
что при удачном разборе переменная \texttt{result} получит вектор пар или
flow-узлов. Выполнить их означает пройтись по всем узлам и передать их в
неизвестный пока \texttt{do-flow}:
...
Подумаем, что такое \texttt{do-flow}. Поскольку для каждого типа узла нужна своя
реализация, логично объявить мультиметод. Он ищет свою реализацию по первому
элементу пары~--- тегу:
...
других действий.  \footurl{Полный список}{https://www.selenium.dev/selenium-ide/docs/en/api/commands}[Commands]
к документации Selenium IDE включает несколько десятков команд.
...
\texttt{:this}, поэтому условие мы бы получили как \texttt{(:expression
  this)}. Пока что не ясно, как проверить условие, поэтому мы оставили
многоточие. Если условие истина, рекурсивно вызываем \texttt{run-flow} для
положительной ветви \texttt{if}. Вложенные \texttt{if} и \texttt{for} будут
обработаны должным образом. Если же условие ложно и при этом заполнена
негативная ветка, выполним блок \texttt{flow} из нее.

Чтобы добиться исполнения кода, пойдем на еще несколько упрощений. Во-первых,
...
При запуске увидим на экране строки:
...
Видим, что сработала только положительная ветка, причем в цикле
\texttt{for}. Разбор и выполнение команд написаны правильно.
...
перехват исключений и отладку. Но наиболее важный шаг~--- разбор команд до их
исполнения~--- рассмотрен нами полностью. Мы описали его спекой без участия
сторонних библиотек, что, на наш взгляд, только доказывает, насколько это
здорово~--- иметь столь мощный инструмент под рукой.
