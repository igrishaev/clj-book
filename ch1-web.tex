...
документах. Документ HTTP называется \footurl{RFC~2616}{https://tools.ietf.org/html/rfc2616}[RFC~2616][-3mm].
...
\verb|format|. Поскольку \verb|app|~--- это функция, вызовем её с запросами:
...
(defmethod multi-handler :page-view
  [request]
  (if-let [order (some-> request :route-params :id
                         get-order-by-id)]
    {:status 200
