...
Классы, унаследованные от \verb|Error| и \texttt{Runtime\zspace{}Exception},~--- это
...
по-разному. Emacs с модулем CIDER откроет буфер \texttt{*cider\zspace-error*}
...
Функции \verb|print-stack-trace| и \texttt|print-cause\zspace{}-trace| печатают трейс с
...
\else
\fi
Структуру выше получили функцией \texttt{Throwable\zspace->map}. Она принимает
...
\else
\fi
Объясним оператор \verb|@| перед вызовом \verb|r/capture!| \ifx\devicetype\mobile\coderef{16}\else\coderef{14}\fi. Функция
...
перехватим другой \texttt{:user-er\zspace{}ror} из чужой библиотеки.
