...
The second argument is the \verb|string?|. A predicate is a function that returns true or false. A function is a building block for a spec. The spec wraps the function into a specific object. Technically, we can refer to it: the \verb|s/get-spec| function will return a spec object by the key. In practice, the object is not needed, because keys are specified everywhere.
...
The reason is that \verb|nil| was plugged into the \verb|re-matches|. The function treats the argument as a string, resulting in \verb|NPE|. Rewrite the spec so that it won't throw exceptions. In the example with \verb|::url|, we should first make sure that it is a string, and only then check with a regular expression.
...
Newbies ask: why not make clear messages at the library level? For example, assign a field to a spec that says — “Please enter the correct address.” Why not take a Python or JavaScript library as a sample?
...
\item
  \verb|:in| is the physical path to the value. It is a vector of keys and indexes that are passed to the \verb|get-in|function. If we execute \verb|(get-in <data> <:in>)|, we'll get the value that caused the error.
...
For \verb|get-message| o return a new message, add the \verb|::email| key to the map:
...
(def spec-errors
  {::email
   "Enter the correct email addressс"})
...
\mnoindent
The \texttt{(s/def :book/description ::ne\--string)} expression is a link. When one spec refers to another, Spec shortens the chain to reach \verb|::ne-string| immediately. In such a situation, they say that the \verb|:book/description| and \verb|:movie/description|  \emph{converge} в \verb|::ne-string|. Sometimes convergence has an advantage: you can specify a translation for \verb|::ne-string| and refer to it in all text fields. When the \verb|:via| path is vital, the spec is reorganized.

\mnoindent
One way to do this is to replace the link with a predicate. Rewrite \verb|:book/description| s in the example below:
...
We should filter out blocked users using the \verb|filter and predicate  \spverb|(complement :blocked)|.
...
\footurl{INI}{https://en.wikipedia.org/wiki/INI\_file}[INI][-3mm]~---
is an old configuration format. It consists of titles and pairs of fields and values. Here's an example of a conditional  \verb|config.ini|:
...
Now, we will describe parameters of a function. It is \verb|s/cat| , which takes a bite out of date arguments and places it in a dictionary with the \verb|:start| and
\verb|:end| keys.
